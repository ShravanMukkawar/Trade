...
const ExchangeRateGraph = () => {
  const [selectedRange, setSelectedRange] = useState(RANGE_OPTIONS[1]); // default 3M
  ...
  const { startDate, endDate } = useMemo(
    () => ({
      startDate: getStartDate(selectedRange.months),
      endDate: getEndDate(),
    }),
    [selectedRange]
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(
          "Unable to load USD → EUR data right now. Please try again later."
        );
        console.error(e);
      } finally {
        setLoading(false);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `usd-eur-${selectedRange.label.toLowerCase()}.png`;
      link.click();
    } catch (e) {
    ...
    <section className="w-full max-w-5xl mx-auto px-2 sm:px-4 md:px-8 py-6 sm:py-8 md:py-10 lg:py-16 text-slate-100">
      <header className="mb-4 sm:mb-6 md:mb-8">
        ...
        <p className="text-xs sm:text-sm md:text-base text-slate-700 max-w-2xl">
          Historical relationship between the US Dollar (USD) and Euro (EUR)
          using live data.
        </p>
      </header>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data} margin={{ top: 10, left: 0, right: 10 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value) => [`${value.toFixed(4)} EUR`, "1 USD"]} // eslint-disable-line
                />
                <Line
                  type="monotone"
                  dataKey="rate"
